...
import com.rjx.expensex.data.model.AccountEntity
import com.rjx.expensex.data.model.BudgetEntity
import com.rjx.expensex.data.model.CategoryEntity
...
class AnalyticsDelegate(
    ...
    val financialSummary: StateFlow<FinancialSummary> = combine(
        ...
        transactions.forEach { tx ->
            ...
            if (isCurrentMonth) {
                if (tx.type == com.rjx.expensex.data.model.TransactionType.INCOME) {
                    totalIncomeMonth += tx.amount
                } else if (tx.type == com.rjx.expensex.data.model.TransactionType.EXPENSE) {
    ...
    val categorySpendingData: StateFlow<List<ChartCategoryData>> = combine(
        ...
        val filtered = transactions.filter { tx ->
            if (tx.type != com.rjx.expensex.data.model.TransactionType.EXPENSE) return@filter false
            val txCal = Calendar.getInstance().apply { timeInMillis = tx.date }
            when (period) {
        ...
        for (i in 5 downTo 0) {
            ...
            transactions.forEach { tx ->
                ...
                if (txCal.get(Calendar.YEAR) == targetYear && txCal.get(Calendar.MONTH) == targetMonth) {
                    if (tx.type == com.rjx.expensex.data.model.TransactionType.EXPENSE) {
                        expenseSum += tx.amount
                    } else if (tx.type == com.rjx.expensex.data.model.TransactionType.INCOME) {
                        incomeSum += tx.amount
                    }
                }
            }
    ...
    val topMerchants: StateFlow<List<MerchantSpending>> = allTransactions.map { transactions ->
        ...
        transactions.filter { it.type == com.rjx.expensex.data.model.TransactionType.EXPENSE && it.merchant.isNotBlank() }
            .forEach { tx ->
    ...
    fun suggestCategoryForMerchant(merchant: String): CategoryEntity? {
        return com.rjx.expensex.domain.usecase.SuggestCategoryUseCase().invoke(
            merchant = merchant,
