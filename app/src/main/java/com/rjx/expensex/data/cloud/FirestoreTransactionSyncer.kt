                        ...
                        for (doc in snapshot.documents) {
                            try {
                                ...
                                val entity = TransactionEntity(
                                    id = existing?.id ?: 0L,
                                    uuid = uuid,
                                    type = (data["type"] as? String)?.let { runCatching { com.rjx.expensex.data.model.TransactionType.valueOf(it.uppercase()) }.getOrNull() } ?: com.rjx.expensex.data.model.TransactionType.EXPENSE,
