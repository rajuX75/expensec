package com.rjx.expensex.data.backup
                ...
                for (i in 0 until transactionsArr.length()) {
                    ...
                    transactionsList.add(
                        TransactionEntity(
                            id = obj.optLong("id", 0),
                            uuid = obj.optString("uuid").ifBlank { UUID.randomUUID().toString() },
                            type = runCatching { com.rjx.expensex.data.model.TransactionType.valueOf(obj.optString("type", "EXPENSE").uppercase()) }.getOrDefault(com.rjx.expensex.data.model.TransactionType.EXPENSE),
